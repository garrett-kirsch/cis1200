...
let assoc_list: (int * string) list =
  [(1100, "Java"); (1200, "OCaml"); (1210, "Java"); (2400, "C")]
...
let rec assoc (key: 'k) (l: ('k * 'v) list) : 'v =
  begin match l with
  | (f, s) :: tl -> if f = key then s else assoc key tl
...
let test () : bool =
  capitalize ["a"; "LIST"; "of"; "words"] = ["A"; "LIST"; "OF"; "WORDS"]
;; run_test "capitalize: already capitalized list works" test
...
let test () : bool =
  concat [] = []
;; run_test "concat: empty returns empty" test
