...
let texts () : (string * string set) list =
  (*[("Modest", set_of_list modest); ("Weak", set_of_list weak);
  ("TwentyK", set_of_list twentyk)]*)
   transform (fun (f,s) -> f, set_of_list s) texts_as_lists
...
let count_words_in_text (vocabulary: string list) (text: string set) : int =
   let in_text (x: string) : bool = member x text in
   count_if in_text vocabulary
...
let test () : bool = 
  count_sat_words (set_of_list []) = 0
;; run_test "count_sat_words: empty set" test

let test () : bool = 
  count_sat_words (set_of_list ["allege"]) = 1
;; run_test "count_sat_words: singleton set" test

let test () : bool = 
  count_sat_words (set_of_list ["asdfasdfasdf"]) = 0
;; run_test "count_sat_words: no match set" test

let test () : bool = 
  count_sat_words (set_of_list sat_vocab) = list_length sat_vocab
;; run_test "count_sat_words: empty set" test
