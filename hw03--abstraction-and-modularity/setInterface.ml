...
(* This module signature defines the values associated with the
   abstract type `'a set`. Any module struct that implements the `SET`
...
module type SET = sig
  ...
  (* `equals s1 s2` returns true exactly when s1 and s2 have the same
     elements.  For example:
       equals {"a", "b"} {"b", "a"} = true
       equals {true} {false} = false *)
  val equals : 'a set -> 'a set -> bool

  (* `set_of_list l` returns a set containing all the elements of the
     list l. *)
  val set_of_list : 'a list -> 'a set

  (* You do not need to test this. This value is for debugging purposes
     only: it allows us to print out the name of the set implementation
     that's currently being tested. *)
  val debug_name: string

  (* In practice, we might want a richer interface for working with
     sets, possibly including operations like "intersection" and
     "union", as well as the set analog of transform and fold.  We
     don't include these operations in this HW, though. *)
end
