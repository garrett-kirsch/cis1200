...
let your_tree: int tree =  (* replace this with your tree *)
  Node (
    Node (
      Node (Empty, 2, Empty),
      4,
...
let rec is_perfect (t: 'a tree) : bool = 
  ...
  | Empty -> true
  | Node (l, _, r) -> 
      tree_height l = tree_height r && is_perfect l && is_perfect r
  end
...
let test () : bool = 
  let imperfect_tree = 
    Node (Node (Node (Empty, 5, Node (Empty, 4, Empty)), 52, 
    Node (Empty, 2, Empty)), 19, Node (Node (Empty, 4, Empty), 16, 
    Node (Empty, 1, Empty))) in
  not (is_perfect imperfect_tree)
;; run_test "is_perfect: imperfect multi-level tree returns false" test

let test () : bool = 
  let imperfect_tree = Node (Node (Empty, 2, Empty), 3, Empty) in
  not (is_perfect imperfect_tree)
;; run_test "is_perfect: imperfect small tree" test
...
let rec tree_max (t: 'a tree) : 'a =
  ...
  | Node (l, x, r) ->
      let max_branches : 'a = 
        ...
        in
      if x > max_branches then x else max_branches
  end
...
let rec lookup (x: 'a) (t: 'a tree) : bool =
  begin match t with 
  | Empty -> false 
  | Node (l, n, r) -> n = x || (if x > n then lookup x r else lookup x l) 
  end
...
let rec insert (x: 'a) (t: 'a tree) : 'a tree =
  ...
  | Node (l, n, r) ->
      if x = n then t 
      else if x > n then Node (l, n, insert x r ) 
      else Node (insert x l, n, r)
  end
...
let rec delete (x: 'a) (t: 'a tree) : 'a tree =
  ...
  | Node (l, n, r) ->
      if n = x then 
        ...
        | Node _, Empty -> l
        | _ -> 
            let maxl = tree_max l in 
            Node (delete maxl l, maxl, r)
        end
      else if x > n then Node(l, n, delete x r)
      else Node(delete x l, n, r)
  end
