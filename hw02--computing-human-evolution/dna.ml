...
let rec complementary_helix (x: helix) : helix =
  begin match x with 
  | [] -> []
  | A :: tl -> T :: (complementary_helix tl)
  | T :: tl -> A :: (complementary_helix tl)
  | G :: tl -> C :: (complementary_helix tl)
  | C :: tl -> G :: (complementary_helix tl)
  end
...
let rec hamming_distance (x1: helix) (x2: helix) : int =
  ...
  | x::xs, y::ys -> if x<>y then 1 + hamming_distance xs ys
                           else hamming_distance xs ys
  | _ -> 0
  end
...
let rec start_list (x: helix) : acid list = 
  begin match x with 
  | a::b::c::tl -> 
      let y : acid = (acid_of_triplet a b c) in
      if y = END then [] else y :: start_list tl
  | _ -> []
  end
...
let rec acids_of_helix (x : helix) : acid list =
  ...
  | a::b::c::tl -> 
      let y : acid = (acid_of_triplet a b c) in
      if y = Met then start_list x else (acids_of_helix (b::c::tl))
  | _ -> []
  end
...
let rec unlabel_tree (t: labeled_tree) : tree =
  begin match t with
  | LLeaf x -> Leaf x
  | LNode (l,x,r) -> Node (unlabel_tree l, unlabel_tree r)
  end
(* -------------------------------------------------------------------------- *)
...
let rec add_ancestor_labels (t: tree) : labeled_tree =
  ...
  | Leaf x -> LLeaf x
  | Node (l, r) -> 
    let new_l : labeled_tree = add_ancestor_labels l in
    let new_r : labeled_tree = add_ancestor_labels r in
    begin match new_l, new_r with 
    | LLeaf x, LLeaf y -> LNode (new_l, guess_parent_helix x y, new_r)
    | LNode (_,x,_), LLeaf y -> LNode (new_l, guess_parent_helix x y, new_r)
    | LLeaf x, LNode (_,y,_)-> LNode (new_l, guess_parent_helix x y, new_r)
    | LNode (_,x,_), LNode (_,y,_) ->LNode(new_l, guess_parent_helix x y, new_r)
    end
  end
...
let rec parent_child_hamming (t: labeled_tree) : int =
  ...
  | LLeaf x -> 0
  | LNode (l,x,r) ->
    let sum_left = parent_child_hamming l in
    let sum_right = parent_child_hamming r in
    let sum = sum_left + sum_right in
    begin match l,r with
    | LLeaf a, LLeaf b -> 
    (hamming_distance x a) + (hamming_distance x b) + sum
    | LLeaf a, LNode (_,b,_) -> 
    (hamming_distance x a) + (hamming_distance x b) + sum
    | LNode (_,a,_), LLeaf b -> 
    (hamming_distance x a) + (hamming_distance x b) + sum
    | LNode (_,a,_), LNode (_,b,_) -> 
    (hamming_distance x a) + (hamming_distance x b) + sum
    end
  end
...
let rec simplest_tree (ts: labeled_tree list) : labeled_tree * int =
  ...
  | x::xs -> 
    let simplest : labeled_tree * int = simplest_tree xs in
    let (f, s) = simplest in
    if parent_child_hamming x > s 
    then simplest 
    else x, parent_child_hamming x
  end 
...
let find_simplest_tree (x1: helix) (x2: helix) (x3: helix) (x4: helix) : tree =
  let (t,_) = simplest_tree(add_ancestor_labels_list (all_trees x1 x2 x3 x4)) in
  unlabel_tree t
