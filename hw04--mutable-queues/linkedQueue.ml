...
module LinkedQueue : QueueInterface.QUEUE = struct
  ...
  let contains (elt: 'a) (q: 'a queue) : bool =
    ...
    let rec loop (no : 'a qnode option) (b : bool) : bool = 
      begin match no with
      | None -> b
      | Some n -> if n.v == elt then loop None true else loop (n.next) b
      end
    in
    loop q.head false
  ...
  let truncate (elt: 'a) (q: 'a queue) : unit =
    ...
    let rec loop (no : 'a qnode option) : unit = 
      begin match no with 
      | None -> ()
      | Some n -> if n.v == elt then (n.next <- None; q.tail <- no) 
                                else loop n.next
      end
    in
    loop q.head
  ...
  let delete (elt: 'a) (q: 'a queue) : unit =
    ...
    let rec loop (cur : 'a qnode option) (prev: 'a qnode option) : unit =
      begin match cur with
      | None -> ()
      | Some n -> 
          if n.v = elt
          then begin match prev with 
          | None -> q.head <- n.next
          | Some n2 -> n2.next <- n.next
          end
          else (loop n.next cur)
      end
    in
    loop q.head None
